...
max_steps = 2000
means = {
    'o': np.load('normalization/pusher_{}/mean_obs.npy'.format(max_steps)),
    's': np.load('normalization/pusher_{}/mean_s_transition_obs.npy'.format(max_steps)),
    'r': np.load('normalization/pusher_{}/mean_r_transition_obs.npy'.format(max_steps)),
    'c': np.load('normalization/pusher_{}/mean_correction.npy'.format(max_steps)),
}

std = {
    'o': np.load('normalization/pusher_{}/std_obs.npy'.format(max_steps)),
    'a': np.load('normalization/pusher_{}/std_actions.npy'.format(max_steps)),
    's': np.load('normalization/pusher_{}/std_s_transition_obs.npy'.format(max_steps)),
    'r': np.load('normalization/pusher_{}/std_r_transition_obs.npy'.format(max_steps)),
    'c': np.load('normalization/pusher_{}/std_correction.npy'.format(max_steps)),
}
...
class Pusher3DofInferenceWrapper(gym.Wrapper):
    ...
    def _step(self, action):
        ...
        return self.env.step((action_real * std['a']).clamp(min=-1., max=1.))
...
def Pusher3DofInverse(base_env_id):
